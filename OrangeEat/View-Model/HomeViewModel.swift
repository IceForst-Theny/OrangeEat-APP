...
class HomeViewModel: NSObject, ObservableObject, CLLocationManagerDelegate {
    ...
    func fetchPandaData(StoreName: String) {
        let store_name: String = StoreName
        let db = Firestore.firestore()
        db.collection("\(store_name)").getDocuments { (snap, err) in
            guard let itemData = snap else { return }
            
            self.items = itemData.documents.compactMap({ (doc) -> Item? in
                let id = doc.documentID
                let name = doc.get("item_name") as! String
                let cost =  doc.get("item_cost") as! Double
    ...
    func fetchDunkinData(StoreName: String) {
        let store_name: String = StoreName
        let db = Firestore.firestore()
        db.collection("\(store_name)").getDocuments { (snap, err) in
            guard let itemData = snap else { return }
    ...
    func fetchFiveGuysData(StoreName: String) {
        let store_name: String = StoreName
        let db = Firestore.firestore()
        db.collection("\(store_name)").getDocuments { (snap, err) in
            guard let itemData = snap else { return }
    ...
    func fetchPopEyesData(StoreName: String) {
        let store_name: String = StoreName
        let db = Firestore.firestore()
        db.collection("\(store_name)").getDocuments { (snap, err) in
            guard let itemData = snap else { return }
    ...
    func calculateTotalPrice() -> String {
        ...
        cartItems.forEach { (item) in
            price += Float64(item.quantity) * item.item.item_cost
        }
    ...
    func updateOrder() {
        ...
        if ordered {
            ordered = false
            db.collection("UsersOrder").document(Auth.auth().currentUser!.uid).delete {
                (err) in
                if err != nil {
                    self.ordered = true
                }
